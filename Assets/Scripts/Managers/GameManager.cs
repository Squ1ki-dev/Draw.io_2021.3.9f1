...
public class GameManager : SingletonMB<GameManager>
{
    ...
    {
        var skinIndex = GetSkinIndex(skin);

        Instance.m_PlayerSkinID = skinIndex;

        int favoriteSkin = Mathf.Min(m_StatsManager.FavoriteSkin, m_Skins.Count - 1);

        m_StatsManager.FavoriteSkin = skinIndex;

        Instance.SetColor(Instance.ComputeCurrentPlayerColor(true, 0));
    }

    public SkinData GetPlayerSkin()
    {
        return m_Skins[m_PlayerSkinID];
    }
}
